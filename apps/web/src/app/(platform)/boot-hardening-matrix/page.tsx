import { JsonPanel } from "@/components/platform/JsonPanel";
import { LowBandwidthNotice } from "@/components/platform/LowBandwidthNotice";
import { SectionHeader } from "@/components/platform/SectionHeader";
import { getBootstrapReport } from "@/lib/deployment/deployment-api";

export const dynamic = "force-dynamic";

export default async function BootHardeningMatrixPage() {
  const bootstrap = await getBootstrapReport();

  return (
    <main className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-4 py-6 md:px-6 lg:px-8">
      <SectionHeader
        eyebrow="Boot Hardening Matrix"
        title="Bootstrap and Startup Hardening Surface"
        description="Inspection surface for bootstrap hardening and startup sequence payloads during final launch hardening."
      />

      <LowBandwidthNotice />

      <section className="grid gap-4 lg:grid-cols-2">
        <JsonPanel
          title="Bootstrap Hardening"
          payload={bootstrap.bootstrapReport.bootstrapHardening}
        />
        <JsonPanel
          title="Startup Sequence"
          payload={bootstrap.bootstrapReport.startupSequence}
        />
      </section>
    </main>
  );
}
