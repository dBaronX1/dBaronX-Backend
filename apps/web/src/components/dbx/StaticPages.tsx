...
import { fetchWebStoreProducts } from "@/lib/store-products-server";
...
export async function DbxHomePage() {
  const initialProducts = (await fetchWebStoreProducts({ limit: 24 })).products;
  return (
    <DbxVisualShell
      title="dBaronX"
...
export async function DbxShopPage({ handle }: { handle?: string }) {
  const initialProducts = (await fetchWebStoreProducts({ limit: handle ? 8 : 24, handle })).products;
  return (
