...
async function safeGraph(
  ...
  try {
    const result = await query.graph({ entity, fields, filters, pagination: { take } })
...
const hasCountryGeoZone = (serviceZone: unknown) =>
  asArray(isRecord(serviceZone) ? serviceZone.geo_zones : undefined).some(
    (geoZone) => isRecord(geoZone) && String(geoZone.type || "").toLowerCase() === "country" && String(geoZone.country_code || "").toLowerCase() === DEFAULT_COUNTRY_CODE
  )

const findUsServiceZone = (fulfillmentSet: unknown): Record<string, unknown> | undefined => {
  const serviceZones = asArray<Record<string, unknown>>(isRecord(fulfillmentSet) ? fulfillmentSet.service_zones : undefined).filter(isRecord)
  return serviceZones.find(hasCountryGeoZone) || serviceZones.find((zone) => getId(zone))
}

async function findStockLocationFulfillmentSet(query: any, stockLocationId: string) {
  ...
  const fulfillmentSetFromLocation = asArray<Record<string, unknown>>(stockLocation?.fulfillment_sets).find((set) => getId(set))
  ...
  return fulfillmentSets.find((set) =>
    asArray<Record<string, unknown>>(set.stock_locations).some((location) => getId(location) === stockLocationId)
  )
}

async function findShippingOption(query: any, serviceZoneId: string | null, shippingProfileId: string | null) {
  const shippingOptions = await safeGraph(
    query,
    "shipping_option",
    ["id", "name", "service_zone_id", "shipping_profile_id", "service_zone.id", "shipping_profile.id"],
    undefined,
    100
  )

  return shippingOptions.find((option) => {
    const optionServiceZoneId = typeof option.service_zone_id === "string" ? option.service_zone_id : getId(option.service_zone)
    const optionShippingProfileId = typeof option.shipping_profile_id === "string" ? option.shipping_profile_id : getId(option.shipping_profile)
...
export async function ensureShippingReadiness(container: any, options: EnsureShippingReadinessOptions = {}): Promise<EnsureShippingReadinessResult> {
  ...
  const region = (await safeGraph(query, "region", ["id", "name", "currency_code", "countries.iso_2"], { id: TARGET_REGION_ID }, 1))[0]
  const regionId = getId(region)
  if (regionId) pushUnique(existing, "region")
  else pushUnique(blockers, "region_missing")

  const shippingProfile = (await safeGraph(query, "shipping_profile", ["id", "name", "type"], { id: TARGET_SHIPPING_PROFILE_ID }, 1))[0]
  ...
  const stockLocation = (await safeGraph(query, "stock_location", ["id", "name"], { id: TARGET_STOCK_LOCATION_ID }, 1))[0]
  ...
  const fulfillmentProvider = (await safeGraph(query, "fulfillment_provider", ["id"], undefined, 20))[0]
  const fulfillmentProviderId = getId(fulfillmentProvider)
  const fulfillmentProviderReady = Boolean(fulfillmentProviderId)
  if (fulfillmentProviderReady) pushUnique(existing, "fulfillment_provider")
  else pushUnique(blockers, "fulfillment_provider_missing")

  let fulfillmentSet = stockLocationId ? await findStockLocationFulfillmentSet(query, stockLocationId) : undefined
  ...
  if (!fulfillmentSetId && repair && stockLocationId) {
    ...
    fulfillmentSet = await findStockLocationFulfillmentSet(query, stockLocationId)
    fulfillmentSetId = getId(fulfillmentSet)
  }

  if (fulfillmentSetId) pushUnique(fulfillmentSet && created.includes("fulfillment_set") ? created : existing, "fulfillment_set")
  ...
  let serviceZoneReady = Boolean(serviceZoneId && hasCountryGeoZone(serviceZone))
  ...
  if (serviceZoneId && !serviceZoneReady && repair) {
    ...
    serviceZone = asArray<Record<string, unknown>>(updated.result).find((zone) => getId(zone) === serviceZoneId) || serviceZone
    pushUnique(created, "service_zone_geo_zone")
    fulfillmentSet = stockLocationId ? await findStockLocationFulfillmentSet(query, stockLocationId) : fulfillmentSet
    serviceZone = findUsServiceZone(fulfillmentSet) || serviceZone
    serviceZoneReady = Boolean(getId(serviceZone) && hasCountryGeoZone(serviceZone))
  }
  ...
  serviceZoneId = getId(serviceZone)
  if (serviceZoneReady) pushUnique(created.includes("service_zone") ? created : existing, "service_zone")
  else pushUnique(blockers, "service_zone_missing")

  let shippingOption = serviceZoneId && shippingProfileId ? await findShippingOption(query, serviceZoneId, shippingProfileId) : undefined
  let shippingOptionId = getId(shippingOption)

  const canCreateShippingOption = Boolean(regionId && shippingProfileId && fulfillmentProviderId && serviceZoneReady && serviceZoneId)
  ...
  if (shippingOptionReady) pushUnique(created.includes("shipping_option") ? created : existing, "shipping_option")
