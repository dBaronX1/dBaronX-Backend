...
function failPrerequisite(reason: string, details: Record<string, unknown> = {}): never {
  console.error(JSON.stringify({ success: false, error: reason, ...details }, null, 2));
  process.exit(1);
}
...
export default async function seedProducts({ container }: ExecArgs) {
  ...
  const existingProducts = asArray<any>(existingProductsResult, ["products"]);

  const existingHandles = new Set(existingProducts.map((p: any) => p?.handle).filter(Boolean));
  ...
  const shippingProfile =
    shippingProfiles.find((sp: any) => sp?.type === "default") || shippingProfiles[0] || null;
  ...
  const diagnostic = {
    ...
    counts: {
      ...
      existingProductHandles: existingHandles.size,
      plannedCreateProducts: candidates.length,
    },
  };

  if (dryRun) {
    logger.info(`Dry-run diagnostics: ${JSON.stringify(diagnostic.counts)}`);
    console.log(JSON.stringify({ dryRun: true, ...diagnostic }, null, 2));
  }

  if (!candidates.length) {
    logger.info(`No products to seed. Source=${source}. Existing handles matched all inputs.`);
    if (!dryRun) console.log(JSON.stringify({ success: true, source, createdCount: 0, skippedCount: products.length }, null, 2));
    return;
  }

  if (!defaultSalesChannel) failPrerequisite("no sales channel found", diagnostic);
  if (!shippingProfile) failPrerequisite("no shipping profile found", diagnostic);
  if (!stockLocation) failPrerequisite("no stock location found", diagnostic);
  ...
  const createdProducts = asArray<any>(created?.result);
  const inventoryLevels = createdProducts.flatMap((product: any, idx: number) => asArray<any>(product?.variants).map((variant: any) => ({ inventory_item_id: asArray<any>(variant?.inventory_items)[0]?.id, location_id: stockLocation.id, stocked_quantity: candidates[idx].variant.inventoryQuantity || 0 }))).filter((x: any) => x.inventory_item_id && x.location_id);
  if (inventoryLevels.length) await createInventoryLevelsWorkflow(container).run({ input: { inventory_levels: inventoryLevels } });

  console.log(JSON.stringify({ success: true, source, createdCount: createdProducts.length, skippedCount: products.length - normalized.length }, null, 2));
}
