...
export class FirstOwnerBootstrapService {
  ...
  async bootstrapFirstOwner(dto: BootstrapFirstOwnerDto, authorization?: string): Promise<BootstrapFirstOwnerResponseDto> {
    if (this.env("DBX_ENABLE_FIRST_OWNER_BOOTSTRAP") !== "true") {
      throw new ForbiddenException({ code: "FIRST_OWNER_BOOTSTRAP_DISABLED", message: "Set DBX_ENABLE_FIRST_OWNER_BOOTSTRAP=true only for the controlled owner-claim window." });
    }

    const expectedToken = this.env("DBX_OWNER_BOOTSTRAP_TOKEN") || this.env("INTERNAL_SERVICE_TOKEN");
    if (!expectedToken) {
      throw new ServiceUnavailableException({ code: "FIRST_OWNER_BOOTSTRAP_TOKEN_NOT_CONFIGURED", message: "Configure DBX_OWNER_BOOTSTRAP_TOKEN or INTERNAL_SERVICE_TOKEN before using first-owner bootstrap." });
    }

    const suppliedToken = this.extractBearer(authorization);
    if (!suppliedToken || suppliedToken !== expectedToken) {
      throw new UnauthorizedException({ code: "FIRST_OWNER_BOOTSTRAP_TOKEN_REQUIRED", message: "A valid bearer token is required." });
    }

    const data = await this.supabase.rpc<Record<string, unknown>>("dbx_bootstrap_first_owner_user", {
    ...
    const publicBaseUrl = this.env("DBX_PUBLIC_APP_URL") || this.env("NEXT_PUBLIC_APP_URL") || this.env("WEB_BASE_URL") || "";
