...
For Stripe-controlled test checkout, the session must be `cs_test_*`. A `cs_live_*` session is live-money mode; never use Stripe test cards on live sessions. Configure Render with `STRIPE_SECRET_KEY` to a test-mode `sk_test_*` value and the matching test endpoint `STRIPE_WEBHOOK_SECRET`, redeploy, and rerun the first Stripe smoke.
