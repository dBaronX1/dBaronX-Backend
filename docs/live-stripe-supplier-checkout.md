...
The batch contains eight buyable `manual_verified_for_checkout` CJ products and one incomplete draft humidifier. The draft remains non-buyable until image, inventory, supplier price, shipping cost, total cost, and selling price are completed. Supplier cost and shipping cost stay in Medusa internal metadata and must not be shown by Rocket or Telegram customer flows. Automatic CJ onboarding remains separate: preview → import → approve-safe → publish-approved/full-safe, one category at a time after cooldown when rate-limited.

The manual seed resolves the storefront sales channel from the live publishable key first. It supports both `dBaronX Live Storefront Publishable Key` and `dBaronX Storefront Publishable Key`, preferring the live storefront key when both exist, and links buyable products to that live publishable-key sales channel. If the older canonical/fallback sales-channel check points at a different channel, the seed reports `canonical_sales_channel_differs_from_live_publishable_key_channel` as a warning rather than failing a successful product upsert. Fatal blockers remain for true failures: no live publishable-key sales channel, missing internal product-to-live-sales-channel links, product-level readiness blockers, or failed Store API visibility when a `MEDUSA_PUBLISHABLE_KEY` is available. If the seed runtime does not have `MEDUSA_PUBLISHABLE_KEY`, it still verifies the internal sales-channel link and reports `storeApiProofSkippedReason: "publishable_key_not_available_to_seed_runtime"` instead of failing solely because live HTTP proof could not be attempted.
