...
3. Confirm the returned `sessionId` starts with `cs_test_`; if it starts with `cs_live_`, stop and do not use a test card.
4. Use Stripe test card `4242 4242 4242 4242`, any future expiry, any CVC, and a valid billing ZIP only for a `cs_test_*` session.
5. After the hosted checkout succeeds, open Stripe Dashboard → Developers → Webhooks → the configured endpoint.
6. Confirm a `checkout.session.completed` delivery reached `POST /api/checkout/stripe/webhook` at `https://dbaronx-api-unified.onrender.com/api/checkout/stripe/webhook`.
7. Confirm the API response for the verified event is `verified: true`, `paymentMarkedPaid: false`, and includes `settlement_pending` until durable settlement is implemented.
