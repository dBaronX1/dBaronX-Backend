...
for (const key of ['mkdir -p artifacts','tee artifacts/cj-operator-output.json','if-no-files-found: warn','if: always()','set -o pipefail']) if(!yaml.includes(key)) throw new Error(`missing ${key}`);
