...
const forbiddenCustomerText = ["Set NEXT_PUBLIC_SUPABASE_URL", "Set NEXT_PUBLIC_SUPABASE_ANON_KEY"];
...
const callbackReady = callbackRoutePresent && (!WEB_BASE_URL || (callbackProbe.status >= 300 && callbackProbe.status < 400));
...
const result = {
  ...
  nextManualStep: blockers.length
    ...
    : WEB_BASE_URL
      ? "Fly Web auth routes and runtime public config are ready for a manual Supabase signup/login test."
