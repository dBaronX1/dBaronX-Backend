...
const priceExists = firstPriceAmount(variant, product) > 0;
const stockExists = stockReady(product, variant);
const imageReady = Boolean(clean(product?.thumbnail) || clean(product?.image) || clean(product?.image_url) || clean(metadata.imageUrl) || clean(metadata.image_url) || (Array.isArray(product?.images) && product.images.some((image) => clean(image?.url))));
...
function supplierSignal(item) { const metadata = metadataOf(item); const variantMetadata = metadataOf(firstVariant(item)); return Boolean(clean(metadata.supplier || metadata.supplier_name || variantMetadata.supplier) || clean(metadata.supplierProductId || metadata.supplier_product_id || metadata.productUrl || metadata.sourceUrl || variantMetadata.supplierProductId || variantMetadata.supplier_product_id) || clean(metadata.supplierSku || metadata.supplier_sku || variantMetadata.supplierSku || variantMetadata.supplier_sku || firstVariant(item)?.sku)); }
function isCheckoutCandidate(item) { const variant = firstVariant(item); const metadata = metadataOf(item); return Boolean(item && metadata.realSupplierProduct === true && metadata.demo === false && supplierSignal(item) && firstPriceAmount(variant, item) > 0 && stockReady(item, variant)); }
function isManualCuratedCjProduct(item) { const metadata = metadataOf(item); const variantMetadata = metadataOf(firstVariant(item)); return Boolean(isCheckoutCandidate(item) && clean(metadata.supplier || variantMetadata.supplier).toLowerCase() === 'cj' && metadata.manualCurated === true && metadata.buyable === true && ['manual_verified_for_checkout', 'verified_for_checkout'].includes(metadata.supplierVerificationStatus)); }
function isExpectedProduct(item) { const metadata = metadataOf(item); const variant = firstVariant(item); const variantMetadata = metadataOf(variant); const supplierProductSignal = clean(metadata.supplierProductId || metadata.supplier_product_id || variantMetadata.supplierProductId || variantMetadata.supplier_product_id || metadata.productUrl || metadata.sourceUrl); const skuSignal = clean(metadata.supplierSku || metadata.supplier_sku || variantMetadata.supplierSku || variantMetadata.supplier_sku || variant?.sku); return Boolean(clean(item?.handle) === EXPECTED.handle && (!clean(metadata.supplier || variantMetadata.supplier) || clean(metadata.supplier || variantMetadata.supplier).toLowerCase() === EXPECTED.supplier) && (!supplierProductSignal || supplierProductSignal.includes(EXPECTED.supplierProductId) || supplierProductSignal === EXPECTED.supplierSku) && (!skuSignal || skuSignal === EXPECTED.supplierSku)); }
