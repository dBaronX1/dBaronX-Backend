...
for (const marker of ["/stories/readiness", "/stories/generate", "providerConfigured", "generationEndpointReady", "persistenceReady", "blockers", "success"]) {
  if (!route.includes(marker)) blockers.push(`fastapi_route_missing_${marker}`);
}
for (const key of ["GEMINI_API_KEY", "GOOGLE_GENERATIVE_AI_API_KEY", "OPENAI_API_KEY", "ANTHROPIC_API_KEY"]) {
...
const secretValuePattern = /(GEMINI_API_KEY|GOOGLE_GENERATIVE_AI_API_KEY|OPENAI_API_KEY|ANTHROPIC_API_KEY)\s*=\s*["'][A-Za-z0-9][^"']+["']/;
