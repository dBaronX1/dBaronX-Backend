...
for (const k of ['workflow_dispatch:','mode:','readiness, preview, import, approve-safe, publish-approved, full-safe','category:','all, electronics, fashion, home-living, beauty, sports, automotive, agriculture, tech, finance','limitPerCategory:','default: \'50\'','dryRun:','readinessExitZero:','artifacts/cj-operator-output.json']) if(!y.includes(k)) throw new Error(`missing ${k}`);
...
console.log(JSON.stringify({success:true,smoke:'cj-github-action-50-per-category'}));
